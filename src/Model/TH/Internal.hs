...
module Model.TH.Internal where
...
genNewtypeT nm hsTy pgTy = do
  ...
  let [name, nameT, unName, a, f, x] =
      ...
      -- However, doing so will make typeclass instances undecidable.
#ifdef BUILD
      [nmField, nmSqlArray] = [''Column, ''SqlArray]
#else
      [nmField, nmSqlArray] = mkName <$> ["Opaleye.Column", "Opaleye.SqlArray"]
#endif
  ...
  insts <- [d|
    ...
    type instance F [$(conT name)] = $(conT nameT) ($(conT nmField) ($(conT nmSqlArray) $(conT pgTy)))
    type instance F (Maybe [$(conT name)]) = $(conT nameT) (Maybe ($(conT nmField) ($(conT nmSqlArray) $(conT pgTy))))

    instance Applicative $(conT nameT) where
      pure = $(conE name)
      $(conP name [varP f]) <*> $(conP name [varP x]) = $(conE name) ($(varE f) $(varE x))

    instance Comonad $(conT nameT) where
      extract = $(varE unName)
      duplicate = $(conE name)
    |]
  isStringInst <- if b
